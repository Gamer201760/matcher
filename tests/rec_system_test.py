...
from repository.recommendation_system.service import Form, Group, RecommendationService
...
class TestRecommendationService(unittest.TestCase):
    ...
    def test_06_get_similar(self):
        ...
        users = [
            Form(user_id="sim_user_1", name="User 1", rooms=2, roommates=1, budget=15000, months=12),
            Form(user_id="sim_user_2", name="User 2", rooms=2, roommates=1, budget=16000, months=12),
            Form(user_id="sim_user_3", name="User 3", rooms=1, roommates=0, budget=8000, months=6),
            Form(user_id="sim_user_4", name="User 4", rooms=3, roommates=3, budget=30000, months=18),
            Form(user_id="sim_user_5", name="User 5", rooms=2, roommates=2, budget=14000, months=9),
        ]
        ...
        for form, score in similar:
            ...
            self.assertNotEqual(form.user_id, "sim_user_1")  # Should not include query user

        # Most similar should be sim_user_2 (very similar preferences)
        most_similar_id = similar[0][0].user_id
        logger.info(f"✓ Most similar to sim_user_1: {most_similar_id} (score: {similar[0][1]:.4f})")

        # Log all results
        for i, (form, score) in enumerate(similar, 1):
            logger.info(f"   {i}. {form.name} (ID: {form.user_id}) - {score*100:.1f}% match")
    ...
    def test_07_send_request_to_group(self):
        ...
        # Create two users
        user1_form = Form(user_id="group_user_1", name="Group User 1", rooms=2, roommates=1, budget=15000, months=12)
        user2_form = Form(user_id="group_user_2", name="Group User 2", rooms=2, roommates=1, budget=16000, months=12)
        ...
        with get_driver() as driver:
            with driver.session() as session:
                check_query = """
                    ...
                    RETURN r
                """
                result = session.run(check_query, user_id="group_user_2", group_id=group_id)
    ...
    def test_08_approve_request(self):
        ...
        # Create two users
        user1_form = Form(user_id="approve_user_1", name="Approve User 1", rooms=2, roommates=2, budget=15000, months=12)
        user2_form = Form(user_id="approve_user_2", name="Approve User 2", rooms=2, roommates=2, budget=16000, months=12)
        ...
        self.service.approve_request("approve_user_1", "approve_user_2", max_roommates=4)
        ...
        with get_driver() as driver:
            with driver.session() as session:
                ...
                """
                result = session.run(check_query, user_id="approve_user_2", group_id=group_id)
    ...
    def test_09_approve_request_makes_group_inactive(self):
        ...
        for i in range(2, 5):
            self.service.approve_request("capacity_user_1", f"capacity_user_{i}", max_roommates=4)
    ...
    def test_10_leave_from_group(self):
        ...
        for i in range(2, 4):
            ...
            self.service.approve_request("leave_user_1", f"leave_user_{i}", max_roommates=5)
    ...
    def test_11_get_group(self):
        ...
        for i in range(2, 4):
            ...
            self.service.approve_request("info_user_1", f"info_user_{i}", max_roommates=5)
        logger.info("✓ Formed group with 3 members")
        ...
        logger.info(f"✓ Group {group_id}: {len(members)} members, active={group.active}")
