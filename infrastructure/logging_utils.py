...
def log_neo4j_query(logger: logging.Logger, query: str, **parameters):
    """
    ...
    if parameters:
        # Mask sensitive data
        safe_params = {
            k: v if k not in ['password', 'auth'] else '***'
...
def log_vector_operation(
    logger: logging.Logger, operation: str, vector_size: int, user_id: str
):
    """
...
def log_similarity_results(
    logger: logging.Logger, query_user: str, results: list, top_k: int
):
    """
